...
export function ResizePanel({ image }: Props) {
  ...
  const handleWidthChange = useCallback((w: number) => {
    if (settings.lockAspect) {
      update({ width: w, height: Math.max(1, Math.round(w / aspect)) });
    } else {
      update({ width: w });
    }
  ...
  const handleHeightChange = useCallback((h: number) => {
    if (settings.lockAspect) {
      update({ width: Math.max(1, Math.round(h * aspect)), height: h });
    } else {
      update({ height: h });
    }
  }, [settings.lockAspect, aspect, update]);
  ...
  const handleExport = useCallback(async () => {
    ...
    setLoading(true);
    const tid = toast.loading("Resizing...");
    try {
      ...
      toast.success(`Resized to ${r.info.width} × ${r.info.height}px`, {
        id: tid,
        action: { label: "Show", onClick: () => revealItemInDir(r.output_path) },
      });
    } catch (e) {
      toast.error("Resize failed", { id: tid, description: String(e) });
    } finally {
      setLoading(false);
    }
  }, [image, settings, setCurrentImage]);
  ...
  return (
    <div className="flex h-full flex-col gap-5 p-4">
      ...
      <div className="flex items-end gap-1.5">
        ...
        <button
          type="button"
          title={settings.lockAspect ? "Unlock aspect ratio" : "Lock aspect ratio"}
      ...
      <label className="flex cursor-pointer items-center gap-2">
        <input
          ...
          className="rounded"
        />
        <span className="text-xs text-muted-foreground">Don't upscale</span>
      </label>
      ...
      <div className="mt-auto space-y-1.5">
        ...
        <p className="text-center text-[10px] text-muted-foreground">Ctrl+S</p>
      </div>
    </div>
  );
}
